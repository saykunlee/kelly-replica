...
<div id="app">
	...
	<div class="row">
		...
		<div class="mg-b-10 col-lg-6 col-md-12 col-sm-12 col-xs-12 d-flex">
			<div class="pd-2 flex-fill">
				...
				<select class="form-control select_3" id="commoncode_2_select" style="width: 100%;" v-model="$app_estimate.lib_estimate.bs_platform" :disabled="!is_admin">>
			...
			<div class="pd-2 flex-fill">
				...
				<div class="form-inline">
					<select class="form-control w-100" v-model="$app_estimate.lib_estimate.is_open">
			...
			<!-- 미리보기 -->
			<div hidden class="pd-2 flex-fill">
	...
	<template v-if="$app_estimate.lib_estimate.room_estimate.room_checked.length > 0">
		<div v-for="(item1, index) in $app_estimate.lib_estimate.room_estimate.room_checked" class="row mg-b-10 ">
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 d-flex flex-row justify-content-between tx-neobd">
				<!-- 숙소명 -->
				<div class="">[{{item1.room_type_nm_short}}]</div>
				<!-- 기간(박)/채/인원 -->
				<div class="">{{ item1.exp_ci_date }}-{{ item1.exp_co_date }}({{ item1.nights }}박)/{{ item1.rooms }}채/{{ item1.guests }}인</div>
				<!-- 금액 -->
				<div class="">{{$common.formatMoney(item1.charge_amount_total)}} 페소<!-- ({{$common.formatMoney(item1.charge_amount_total_kr)}}원) --></div>
			</div>
			...
			<table class="table table-bordered room_list mg-b-0 tx-center">
				...
				<tbody>
					<tr v-for="(item_room, index_room) in $app_estimate.lib_estimate.room_estimate.room_checked[0].detail_rooms" v-if="item_room.is_master == '1'">
						...
						<td><a class="btn btn-xs btn-outline-success" @click="open_assign_room_modal($app_estimate.lib_estimate.room_estimate.res_info.reservation_no,item_room.room_group,item_room.room_no)" href="javascript:void(0);">객실변경 </a></td>
		...
		<div class="row mg-t-10">
			<div class="col-12">
				...
				<table class="table">
					<tfoot class="tfoot-estmate bd-0">
						<tr>
							<th class="tx-right wd-50p">숙박 총 비용</th>
							<th class="" colspan="12">
					...
					<div class="media-body">
						<table class="table mg-b-0">
									...
									<tr v-for="(car, index) in $app_estimate.lib_estimate.rental_car_estimate" :key="index">
										<td colspan="13">
											<div class="row">
												...
												<div class="pd-0 col-lg-6 col-md-12 col-sm-12 col-xs-12 d-flex">
													<!-- 차량대수 -->
													<div class="pd-2 flex-fill">
													...
													<!-- 삭제 -->
													<div class="pd-2 align-self-center ">
														<a class="btn btn-outline-danger btn-icon btn-block " href="javascript:void(0);" @click="$app_estimate.deleteRow($app_estimate.lib_estimate.rental_car_estimate,index)"><i class="far fa-trash-alt"></i></a>
													</div>
												</div>
											</div>
										</td>
							...
							<tfoot class="tfoot-estmate">
								<tr>
									<th class="tx-right wd-50p">차량 렌트 총 비용</th>
									<th colspan="12" class="">
										<div class="text-right"><!-- right -->
											<div class="font-size-sm pt-2 ">
												<span v-if="$app_estimate.lib_estimate.rental_car_amount_kr" class="text-muted mr-2">{{$common.formatMoney($app_estimate.lib_estimate.rental_car_amount_kr)}} 원</span>
												<span v-else class="text-muted mr-2">{{$common.formatMoney($app_estimate.exchange_kor($app_estimate.lib_estimate.rental_car_amount))}} 원</span>
											</div>
			...
			<div class="media">
				<div class="media-body">
					<table class="table mg-b-0">
									...
									<td colspan="13">
										<div class="row">
											...
											<div class="pd-0 col-lg-3 d-flex">
												<!-- 골프장 선택 -->
												<div class="pd-2 flex-fill">
													<select class="custom-select" @change="$app_estimate.setGolfSelected(index)" v-model="$app_estimate.lib_estimate.golf_estimate[index].golf_no">
														...
														<template v-else>
															...
															<option v-for="(code, idx) in $app_estimate.getGolfCostList(index)" :value="code.no">{{ code.golf_club_nm }}</option>
														</template>
													</select>
												</div>
											</div>
											<div class="pd-0 col-lg-6 col-md-12 col-sm-12 col-xs-12 d-flex">
												<!-- 인원 -->
												<div class="pd-2 flex-fill">
													<select class="custom-select tx-center" @change="$app_estimate.setGolfSelected(index)" v-model="$app_estimate.lib_estimate.golf_estimate[index].golf_people">
												...
												<!-- 차량금액 -->
												<div class="pd-2 align-self-center ">
													<a class="btn btn-outline-danger btn-icon btn-block " href="javascript:void(0);" @click="$app_estimate.deleteRow($app_estimate.lib_estimate.golf_estimate,index)"><i class="far fa-trash-alt"></i></a>
												</div>
											</div>
										</div>
									</td>
						...
						<tfoot class="tfoot-estmate">
							<tr>
								<th class="tx-right wd-50p">골프 총 비용</th>
								<th colspan="12" class="">
									<div class="text-right"> <!-- right -->
										<div class="font-size-sm pt-2 ">
											<span v-if="$app_estimate.lib_estimate.golf_amount_kr > 0" class="text-muted mr-2">{{$common.formatMoney($app_estimate.lib_estimate.golf_amount_kr)}} 원</span>
											<span v-else class="text-muted mr-2">{{$common.formatMoney($app_estimate.exchange_kor($app_estimate.lib_estimate.golf_amount))}} 원</span>
										</div>
	...
	<div class="mg-b-0 d-flex flex-row justify-content-between">
		<div class="align-self-center">
			<h5 class="mg-t-10" style="font-family:'NanumSquareNeo BD'"> 기타여행견적 </h5>
		</div>
		<div class="align-self-center"><a class="btn btn-sm btn-outline-primary" href="javascript:void(0);" @click="$app_estimate.addItemRow"> <i data-feather="plus" class="mg-r-5"></i>기타여행추가</a></div>
	</div>
	<div class="collapse mg-t-5 mb-3 show" id="Selection-rentcar">
		<div class="row mt-2">
			<!-- list area start -->
				...
				<div class="media">
					<div class="media-body">
						<table class="table mg-b-0">
							...
							<tbody>
								<tr v-if="$app_estimate.lib_estimate.item_estimate.length < 1">
									<td colspan="13" class="pd-l-0 pd-r-0">
										<div class="alert alert-info" role="alert"> 기타여행을 추가해주세요. </div>
									</td>
								</tr>
								<tr v-else v-for="(item, index) in $app_estimate.lib_estimate.item_estimate" :key="index">
									<td colspan="13">
										<div class="row">
											<div class="pd-0 col-lg-2 col-md-12 col-sm-12 col-xs-12 d-flex">
												<!-- 이용날짜 -->
												<div class="pd-2 flex-fill">
													<select class="custom-select tx-center" v-model="$app_estimate.lib_estimate.item_estimate[index].date">
														<option value="">이용날짜 선택하세요.</option>
														<option v-for="(code, idx) in $app_estimate.dateRange" :value="code">{{ code }}</option>
													</select>
												</div>
											...
											<div class="pd-0 col-lg-6 col-md-12 col-sm-12 col-xs-12 d-flex">
												<!-- 액티비티선택 -->
												<div class="pd-2 flex-fill">
													<select class="custom-select" @change="$app_estimate.onItemChange(index)" v-model="$app_estimate.lib_estimate.item_estimate[index].item_no">
														<option value="0" selected>기타여행을 선택해주세요.</option>
														<!-- <option v-for="(code, idx) in rental_car_list" :value="code.no">{{ code.name }}-{{ code.cost_per_night }}페소</option> -->
														<option v-for="(code, idx) in activity_list" :value="code.no">{{ removeHtmlTags(code.cit_search_name) }}</option>
													</select>
												</div>
												<!-- 옵션 -->
												<div v-for="(estimate_item_detail, index_detail) in item.estimate_item_detail" :key="index_detail" class="pd-2 flex-fill tx-center align-self-center">
													<div class="custom-control custom-checkbox">
														<input type="checkbox" class="custom-control-input" :id="`item_detail_check${index}${index_detail}`" :true-value="1" :false-value="0" v-model="estimate_item_detail.useing" @change="$app_estimate.updateItemAndDetailAmount(index)">
														<label class="custom-control-label" :for="`item_detail_check${index}${index_detail}`">{{estimate_item_detail.cde_title}}</label>
													</div>
												</div>
												<!-- !옵션 -->
											</div>
											<div class="pd-0 col-lg-4 col-md-12 col-sm-12 col-xs-12 d-flex">
												<!-- 인원 -->
												<div class="pd-2 flex-fill">
													<select class="custom-select tx-center" @change="$app_estimate.updateItemAndDetailAmount(index)" v-model="$app_estimate.lib_estimate.item_estimate[index].people">
														<option v-for="number in 30" :value="number">{{ number }}</option>
													</select>
												</div>
												<!-- 금액 -->
												<div class="pd-2 flex-fill">
													<div class="input-group">
														<span class="form-control text-right">{{ $common.formatMoney($app_estimate.lib_estimate.item_estimate[index].item_amount) }}</span>
														<div class="input-group-append">
															<span class="input-group-text">페소</span>
														</div>
													</div>
												...
												<!-- 삭제버튼 -->
												<div class="pd-2 align-self-center ">
													<a class="btn btn-outline-danger btn-icon btn-block " href="javascript:void(0);" @click="$app_estimate.deleteRow($app_estimate.lib_estimate.item_estimate,index)"><i data-feather="trash-2" class="mg-r-5"></i></a>
												</div>
											</div>
										</div>
									</td>
							...
							<tfoot class="tfoot-estmate">
								<tr>
									<th class="tx-right wd-50p">기타여행 총 비용</th>
									<th colspan="12" class="">
										<div class="text-right"><!-- right -->
											<div class="font-size-sm pt-2 ">
												<span v-if="$app_estimate.lib_estimate.item_amount > 0" class="text-muted mr-2">{{$common.formatMoney($app_estimate.lib_estimate.item_amount_kr)}} 원</span>
												<span v-else class="text-muted mr-2">{{$common.formatMoney($app_estimate.exchange_kor($app_estimate.lib_estimate.item_amount))}} 원</span>
											</div>
											<h5 class="product-card-title font-weight-bold border-0 pt-0 pd-10 mb-0">{{$common.formatMoney($app_estimate.lib_estimate.item_amount)}} 페소 </h5>
										</div>
									</th>
								</tr>
							</tfoot>
	...
	<div class="mg-b-0 d-flex flex-row justify-content-between">
		<div class="align-self-center">
			<h5 class="mg-t-10" style="font-family:'NanumSquareNeo BD'"> 할인 </h5>
		</div>
	</div>
	<div class="collapse mg-t-5  show" id="Selection-rentcar">
		<div class="row mt-2">
			<!-- list area start -->
			<div class="col-lg-12 ">
				<div class="media">
										...
										<div class="row ">
											<div class="col-lg-5 d-flex">
												<div class="form-group mg-r-5">
													<label>관리자 할인 </label>
													<div class="input-group">
														<input type="text" class="form-control tx-right" v-model="$app_estimate.lib_estimate.dc_amount" placeholder="할인금액 입력">
														<div class="input-group-append">
															<span class="input-group-text">페소</span>
														</div>
													</div>
												...
												<div class="form-group flex-fill ">
													<label>할인 메모</label>
													<input type="text" class="form-control" v-model="$app_estimate.lib_estimate.remark_2" placeholder="할인메모 입력">
												</div>

											</div>
											<div class="col-lg-5 d-flex">
												<div class="form-group mg-r-5">
													<label>관리자 금액조정 </label>
													<div class="input-group">
														<input type="text" class="form-control tx-right" v-model="$app_estimate.lib_estimate.adjust_amount" placeholder="관리자 금액조정 입력">
														<div class="input-group-append">
															<span class="input-group-text">페소</span>
														</div>
													</div>
												...
												<div class="form-group flex-fill ">
													<label>금액조정 메모</label>
													<input type="text" class="form-control" v-model="$app_estimate.lib_estimate.remark_3" placeholder="금액조정메모 입력">
												</div>
											...
											<div class="col-lg-2 d-flex">
												<div class="form-group mg-r-5">
													<label>할인금액 환율</label>
													<div class="input-group">
														<input type="number" class="form-control tx-right" v-model="$app_estimate.lib_estimate.dc_exchange" step="0.1" placeholder="할인금액 환율 입력">
	...
	<!-- <hr class="mg-t-0"> -->
	<table class="table mg-b-0 mg-t-0">
		<tfoot class="tfoot-estmate">
			<tr>
				<th class="tx-right wd-50p">총 할인 비용</th>
				<th colspan="12" class="">
					<div class="text-right"><!-- right -->
						<div class="font-size-sm pt-2 "><span class="text-muted mr-2">{{$common.formatMoney($app_estimate.lib_estimate.total_dc_amount_kr)}} 원</span></div>
						<h5 class="product-card-title font-weight-bold border-0 pt-0 pd-10 mb-0">{{$common.formatMoney($app_estimate.lib_estimate.total_dc_amount)}} 페소 </h5>
					</div>
				</th>
			</tr>
			<tr>
				<th class="tx-right wd-50p">
					<h5 class="tx-neobd">총 여행비용 </h5>
				</th>
				<th colspan="12" class="">
					<div class="text-right"> <!-- right -->
						<template v-if="$app_estimate.lib_estimate.dc_amount > 0 || $app_estimate.lib_estimate.adjust_amount > 0">
							<div class="font-size-sm"><span class="mr-3">{{$common.formatMoney($app_estimate.lib_estimate.pre_amount)}} 페소</span></div>
							<div v-if="$app_estimate.lib_estimate.dc_amount > 0" class="font-size-sm"><span class="tx-danger mr-3">- {{$common.formatMoney($app_estimate.lib_estimate.dc_amount)}} 페소</span></div>
							<div v-if="$app_estimate.lib_estimate.adjust_amount > 0" class="font-size-sm"><span class="tx-success mr-3">- {{$common.formatMoney($app_estimate.lib_estimate.adjust_amount)}} 페소</span></div>
							<hr>
						</template>
						<div class="font-size-sm">
							<span v-if="$app_estimate.lib_estimate.total_amount_kr > 0" class="text-muted mr-3">{{$common.formatMoney($app_estimate.lib_estimate.total_amount_kr)}} 원</span>
							<span v-else class="text-muted mr-3">{{$common.formatMoney($app_estimate.exchange_kor($app_estimate.lib_estimate.total_amount))}} 원</span>
						</div>
	...
	<div class="mg-b-0 d-flex flex-row justify-content-between">
		<div class="align-self-center">
			<h5 class="mg-t-10" style="font-family:'NanumSquareNeo BD'"> 수정이력 </h5>
		</div>
	</div>
	<div class="row mg-t-10">
		<div class="col-lg-12">
	...
	<button v-if="action_type == 'update' " href="#modal_delete" data-toggle="modal" class="btn btn-danger ">견적삭제</button>
	<button href="#modal_save" data-toggle="modal" class="btn btn-success">저장하기</button>
	<a class="btn btn-primary" href="#" data-toggle="modal" @click="openPopupVue('/invoice/'+$app_estimate.lib_estimate.no + '/1')"> <i data-feather="clipboard" class="mg-r-5"></i>미리보기</a>
