   ...
   const modalApp = Vue.createApp({
        ...
        methods: {
            saveMenu() {
                ...
                this.handleAxiosRequest('/api/menu-api/update-top-menu', params, '메뉴 정보가 성공적으로 업데이트되었습니다.', 'no');
                //$('#modal_save').modal('hide');
            },
            deleteMenu() {
                let params = new FormData();
                params.append('no', this.selectedNo);
                this.handleAxiosRequest('/api/menu-api/delete-top-menu', params, '메뉴가 성공적으로 삭제되었습니다.', 'no');
                //$('#modal_save').modal('hide');
            },
            createMenu() {
                let params = this.createFormData(this.details);
                this.handleAxiosRequest('/api/menu-api/create-top-menu', params, '메뉴가 성공적으로 생성되었습니다.', 'no');
              //  $('#modal_save').modal('hide');
            },
            openModal(no) {
                this.selectedNo = no;
                if (no === null) {
                    ...
                    this.fetchDetails('/api/menu-api/get-top-menu-details', no, (data) => {
                        this.details = data;
                    }, 'no');
                }
            },
