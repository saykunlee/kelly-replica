    ...
    const modalApp = Vue.createApp({
        ...
        methods: {
            save() {
                ...
                this.handleAxiosRequest('/api/menu-api/update-sidecate', params, '카테고리 정보가 성공적으로 업데이트되었습니다.', 'no');
                //$('#modal_save').modal('hide');
            },
            deleteDetails() {
                let params = new FormData();
                params.append('no', this.selectedNo);
                this.handleAxiosRequest('/api/menu-api/delete-sidecate', params, '메뉴가 성공적으로 삭제되었습니다.', 'no');
                //$('#modal_save').modal('hide');
            },
            create() {
                let params = this.createFormData(this.details);
                this.handleAxiosRequest('/api/menu-api/create-sidecate', params, '카테고리가 성공적으로 생성되었습니다.', 'no');
                //$('#modal_save').modal('hide');
            },
            openModal(no) {
                this.selectedNo = no;
                if (no === null) {
                    ...
                    this.fetchDetails('/api/menu-api/get-sidecate-details', no, (data) => {
                        this.details = data;
                    }, 'no');
                }
            },
